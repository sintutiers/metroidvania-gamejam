...
func get_cursor_pos() -> Vector2i:
	var pos := (map_overlay.get_local_mouse_position() - MetSys.CELL_SIZE / 2).snapped(
		MetSys.CELL_SIZE
	) / MetSys.CELL_SIZE as Vector2i - map_offset
	return pos
...
func on_layer_changed(l: int):
	...
	if not new_map_view:
		...
		new_map_view = MetSys.make_map_view(
			map,
	...
	if event is InputEventMouseMotion:
		...
		if view_drag != Vector4():
			if event.position.x >= zoomed_overlay.x:
				map_view_container.warp_mouse(
					Vector2(event.position.x - zoomed_overlay.x, real_mouse.y)
				)
				view_drag.x -= zoomed_overlay.x
			elif event.position.x < 0:
				map_view_container.warp_mouse(
					Vector2(map_overlay.size.x + event.position.x, real_mouse.y)
				)
				view_drag.x += zoomed_overlay.x

			if event.position.y >= zoomed_overlay.y:
				map_view_container.warp_mouse(
					Vector2(real_mouse.x, event.position.y - zoomed_overlay.y)
				)
				view_drag.y -= zoomed_overlay.y
			elif event.position.y < 0:
				map_view_container.warp_mouse(
					Vector2(real_mouse.x, map_overlay.size.y + event.position.y)
				)
				view_drag.y += zoomed_overlay.y

			map_offset = Vector2(view_drag.z, view_drag.w) + (map_overlay.get_local_mouse_position()
			- Vector2(view_drag.x, view_drag.y)) / MetSys.CELL_SIZE
			update_map_position()
			map_overlay.queue_redraw()
		else:
			map_overlay.queue_redraw()
