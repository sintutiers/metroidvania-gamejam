...
func undo_handle_group_add(coords: Vector3i, group_id: int):
	...
	editor.undo_redo.add_do_method(
		func():
			MetSys.map_data.cell_groups[group_id].append(coords),
	)
	editor.undo_redo.add_undo_method(
		func():
			MetSys.map_data.cell_groups[group_id].erase(coords),
	)
	had_undo_change = true
...
func undo_handle_element_remove(
	coords: Vector3i,
	element: MetroidvaniaSystem.MapData.CustomElement,
):
	if not undo_active:
		undo_begin()
...
func undo_handle_scene_add(room: Array[Vector3i], old_scene: String, undo_only := false):
	...
	var new_scene: String = MetSys.map_data.get_cell_at(room[0]).scene
...
func _editor_draw(map_overlay: CanvasItem):
	...
	if highlighted_border > -1:
		if highlighted_room.is_empty():
			...
			for p in highlighted_room:
				var cell_data: MetroidvaniaSystem.MapData.CellData = MetSys.map_data.get_cell_at(p)
				for i in 4:
