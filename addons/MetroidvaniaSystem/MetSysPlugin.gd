...
func _enter_tree() -> void:
	...
	for file in DirAccess.get_files_at("res://addons/MetroidvaniaSystem/Translations"):
		if file.get_extension() == "po":
			translation_list.append(
				load("res://addons/MetroidvaniaSystem/Translations".path_join(file))
			)
	...
	metsys_tools.set_item_tooltip(
		-1,
		"Runs get_object_id() for the selected Node and prints the result.",
	)
	metsys_tools.add_item("Copy Object ID to Clipboard")
	metsys_tools.set_item_tooltip(
		-1,
		"Runs get_object_id() for the selected Node and copies the result to clipboard.",
	)
	...
	EditorInterface.get_command_palette().add_command(
		"Print Object ID",
		"met_sys/print_object_id",
		on_tool_option.bind(TOOL_PRINT_ID),
	)
	EditorInterface.get_command_palette().add_command(
		"Copy Object ID to Clipboard",
		"met_sys/copy_object_id_to_clipboard",
		on_tool_option.bind(TOOL_COPY_ID),
	)

	get_singleton().settings.theme_changed.connect(
		func():
			prev_theme_state.clear(),
	)
		...
		TOOL_PRINT_ID, TOOL_COPY_ID:
			...
			if not currrent_scene:
				EditorInterface.get_editor_toaster().push_toast(
					tr("No scene open to check Node."),
			...
			if selection.size() != 1:
				EditorInterface.get_editor_toaster().push_toast(
					tr("You need to select a single Node."),
					EditorToaster.SEVERITY_ERROR,
				)
				return
			...
			if id.is_empty():
				...
				if idx == TOOL_PRINT_ID:
					...
					EditorInterface.get_editor_toaster().push_toast(
						tr("ID \"%s\" copied to clipboard.") % id
					)
...
func POT_hack():
	tr("Error!")
	tr(
		"MetSys restart failed, the singleton still can't be loaded. Make sure the \"MetroidvaniaSystem.gd\" script has no errors.\n\nCommon cause of errors are:\n- Using Godot version older than 4.4.\n- Name conflicts with one of the classes.\n\nThe plugin will be now disabled. Fix the errors and try again."
	)
