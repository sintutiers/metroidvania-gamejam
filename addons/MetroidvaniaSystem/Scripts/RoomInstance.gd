...
func _register_portals() -> void:
	var owner_node := owner if owner else self
	_register_portals_recursive(owner_node, owner_node.scene_file_path)


func _register_portals_recursive(node: Node, scene_path: String) -> void:
	for child: Node in node.get_children():
		if child is Portal:
			LevelAndStateManager.register_portal(child.portal_id, scene_path)
		_register_portals_recursive(child, scene_path)
	...
	for coords in cells:
		...
		for i in 4:
			...
			if cell_data.borders[i] > 0:
				...
				var neicell: MetroidvaniaSystem.MapData.CellData = MetSys.map_data.get_cell_at(
					coords + Vector3i(fwd.x, fwd.y, 0)
				)
				if neicell:
			...
			if show_neightbor:
				...
				preview.add_room(
					temp_map,
					i,
					next_min_cell - Vector2i(next_coords.x, next_coords.y),
				)
